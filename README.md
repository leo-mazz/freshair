...
with the server 'Megatron'. These are
- `/api/user/:username` will display information about a given user, including
all the shows they host, if any. This will allow the Broadcast Controls served
by Megatron to allow a user giving their username to pick a show they want to
broadcast before going on-air.
- `/api/broadcast/on-air` takes POST requests with information, that should be
sent by Megatron, about a show going on air (these will contain a time, the slug
of a show and an on-air id, that should be uniquely generated by Megatron).
If the website already has an on-air record with that id, or if it doesn't know
about a show with that slug, or if the time format is not correct,
it will ignore the request. Otherwise, it will create a new on-air record with
that information. Also, the website will update its current broadcast information
and add the 'On air record' to an existing daily report or to a newly
created daily report if there is not one already. If the request happens when
the website thought another show was on air, the
- `/api/broadcast/off-air` takes POST requests and allows the website to update
the last 'On air record' by adding an end time to it.
It also tells the website that no show is on air.
If the id of the off-air request doesn't match that of any existing on-air
record, a new on-air record is created with nil start time.
